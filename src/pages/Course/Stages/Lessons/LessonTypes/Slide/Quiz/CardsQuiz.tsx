...
interface IProps {
    ...
    onPause: () => any;
    onSubmit: () => any;
    onAnswer: (x: number, y: number) => any;
...
export default class CardsQuiz extends Component<IProps, IState> {
    ...
    handleAnswer = (questionIndex: number, userAnswer: number) => {
        let { answerInc = 0 } = this.context.cardsQuiz;
        const { answers } = this.context.cardsQuiz;
        ...
        return (
            <>
                ...
                <Modal className='qm-timeout' show={showTimeout} onHide={() => {}}>
                    <Modal.Body>
                        <span className='center'>Your time has expired</span>
                    </Modal.Body>
                    <Modal.Footer>
