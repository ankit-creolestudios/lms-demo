...
import LessonInput from '../Courses/Tabs/Course/LessonInput';
import AddNewItemBar from '../Courses/Tabs/AddNewItemBar';
...
function CaretToggle() {
    ...
    return (
        <Button variant='link' type='button'>
            <i className={currentEventKey ? 'fa fa-caret-up' : 'fa fa-caret-down'} />
        </Button>
    );
}

class CoreLibraryContent extends Component {
    state = {
        isLoading: false,
        chapters: [],
    ...
    loadData = async () => {
        if (this.props.parentDocId) {
            this.setState({ isLoading: true });
            const { success, response } = await Api.call('GET', `/core/chapters/core/${this.props.parentDocId}`);
    ...
    submitAction = async () => {
        ...
        const { success, message } = await Api.call('PATCH', '/core/lessons/bulk', {
    ...
    handleSubmit = async (e) => {
        ...
        if (bulkAction === 'DELETE_SELECTED') {
            this.setState({
                ...
                modalAction: async () => {
                    const { success, message } = await Api.call('DELETE', `/core/lessons/${checkedLessons.join(',')}`);
    ...
    submitLesson = async (chapter, lesson, prop) => {
        const { success, message } = await Api.call('PATCH', `/core/lessons/${lesson._id}`, { [prop]: lesson[prop] });
    ...
    submitChapter = async (chapter) => {
        const { success, message } = await Api.call('PUT', `/core/chapters/${chapter._id}`, {
            title: chapter.title,
            ...
            if (chapterId === chapter._id) {
                ...
                chapter.lessons = lessons.map((lesson, i) => {
                    if (lesson.menuIndex !== i) {
                        lesson.menuIndex = i;
                        this.submitLesson(chapter, lesson, 'menuIndex');
                    }
                    return lesson;
                });
            }
    ...
    delete = async (type, docId) => {
        const url = `/core/${type}s/${docId}`;
        const { success, response, message } = await Api.call('DELETE', url);
    ...
    clone = async (type, docId) => {
        const url = `/core/${type}s/${docId}/clone`;
        const { success, response, message } = await Api.call('POST', url);
    ...
    addNewItem = async (title, type, parentId, menuIndex) => {
        const url = `/core/${type}s`;
        const parentType = type === 'lesson' ? 'coreChapterId' : 'coreLibraryId';
        const { success, response, message } = await Api.call('POST', url, {
            [parentType]: parentId,
            title,
            menuIndex,
            ...
            return (
                <div>
                                                    ...
                                                    {(provided) => (
                                                        ...
                                                        >
                                                            ...
                                                            <Accordion.Collapse eventKey={chapter._id}>
                                                                <Card.Body>
                                                                    <Table
                                                                        handleDragAndDrop={this.handleDragAndDropLesson}
                                                                        highlightRow={(row) => {
                                                                                    ...
                                                                                    return (
                                                                                        ...
                                                                                        >
                                                                                            ...
                                                                                            >
                                                                                                <Link
                                                                                                    className='btn'
                                                                                                    to={`/admin/core-library/${this.props.parentDocId}/chapters/${chapter._id}/lessons/${row._id}`}
                                                                                                >
                    ...
                    <AddNewItemBar
                        ...
                        parentId={this.props.parentDocId}
